...
function [output] = RunPractice(window, SearchType, FeatureType, userStruct, structParams, visualTypes)
...
% Calculate the base position of all 12 possible circle locations.
baseLocationsX = radius * cos(2*pi* [1:12] ./12) + xCenter;
baseLocationsY = radius * sin(2*pi* [1:12] ./12) + yCenter;

% Rotate by three elements such that locations match "clock" orientation.
baseLocationsX = circshift(baseLocationsX, 3);
baseLocationsY = circshift(baseLocationsY, 3);
...
for trialNum = 1:numTrials
    ...
    waitTimeout = GetSecs() + structParams.MinTimePerWait;
