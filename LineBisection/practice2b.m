%%% Practice round 2b---line is flashed quickly and then they make their decision 
...
function [wPtr,wOff] = practice2b(subj, blockNum, blockType, wPtr, wOff, rect)
    ...
    for trial = 1:nTrials
        ...
        exp.StimDurationPract    = 1;  %%%This tells how long the line will appear on the screen before disappearing
        keyisdown=0;
        ...
        while GetSecs - StimulusOnset(trial) < exp.StimDurationPract
            ...
            if find(keyCode)==41; Screen('CloseAll'); ListenChar(0); return; end
        end
